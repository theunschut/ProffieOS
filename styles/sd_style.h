...
template<int N>
// Helper: convert any color type to RGBA_um
static inline RGBA_um toRGBA_um_safe(const RGBA_um& x) { return x; }
static inline RGBA_um toRGBA_um_safe(const RGBA_um_nod& x) { return RGBA_um(x); }
static inline RGBA_um toRGBA_um_safe(const SimpleColor& x) { return RGBA_um(x); }
static inline RGBA_um toRGBA_um_safe(const OverDriveColor& x) { return RGBA_um(x); }
static inline RGBA_um toRGBA_um_safe(const RGBA& x) { return RGBA_um(x.c, x.overdrive, x.alpha); }
...
public:
  ...
  RGBA_um getColor(A a, B b, int led) {
    return rt_trans_args[N]->getColor(toRGBA_um_safe(a), toRGBA_um_safe(b), led);
  }
};
...
public:
  RtRgb(int r, int g, int b) : r_(r), g_(g), b_(b), color_(makeRGBA((uint8_t)r, (uint8_t)g, (uint8_t)b)) {
  }
  bool run(BladeBase* blade) override {
...
public:
  ...
  RGBA_um getColor(int led) override {
    ...
    RGBA_um base = children_[0]->getColor(led);
    if (count_ == 1) return base;
    // Compose layers on top via << operator (returns RGBA, not RGBA_um)
    RGBA result(base);
    for (int i = 1; i < count_; i++) {
      RGBA_um layer_color = children_[i]->getColor(led);
      result = result << layer_color;
    }
    // Convert RGBA back to RGBA_um
    return RGBA_um(result.c, result.overdrive, result.alpha);
  }
...
public:
  ...
  RGBA_um getColor(RGBA_um a, RGBA_um b, int led) override {
    ...
    { RGBA _m = MixColors(a, b, mix, 8); return RGBA_um(_m.c, _m.overdrive, _m.alpha); }
  }
private:
  int millis_;
  uint32_t fade_;
...
public:
  ...
  RGBA_um getColor(RGBA_um a, RGBA_um b, int led) override {
    { RGBA _m = MixColors(a, b, fade_, 14); return RGBA_um(_m.c, _m.overdrive, _m.alpha); }
  }
private:
  int millis_;
  int fade_;
...
public:
  ...
  RGBA_um getColor(int led) override {
    ...
    RGBA _m = MixColors(ca, cb, alpha, 15);
    RGBA_um mixed(_m.c, _m.overdrive, _m.alpha);
...
public:
  ...
  RGBA_um getColor(int led) override {
    ...
    if (h < 256) {
      // black -> c1
      { RGBA _m = MixColors(RGBA_um::Transparent(), c1, h, 8); return RGBA_um(_m.c, _m.overdrive, _m.alpha); }
    } else if (h < 512) {
      { RGBA _m = MixColors(c1, c2, h - 256, 8); return RGBA_um(_m.c, _m.overdrive, _m.alpha); }
    } else if (h < 768) {
      RGBA_um white = makeRGBA(255, 255, 255);
      { RGBA _m = MixColors(c2, white, h - 512, 8); return RGBA_um(_m.c, _m.overdrive, _m.alpha); }
    } else {
...
public:
  ...
  RGBA_um getColor(int led) override {
    ...
    { RGBA _m = MixColors(c_->getColor(idx), c_->getColor(idx2), fraction, 15); return RGBA_um(_m.c, _m.overdrive, _m.alpha); }
  }
private:
  RtFuncNode* f_;
  RtColorNode* c_;
...
public:
  RtRgb16(int r, int g, int b) : r_(r), g_(g), b_(b), color_(RGBA_um(Color16((uint16_t)r, (uint16_t)g, (uint16_t)b), 0, 32768)) {
  }
  bool run(BladeBase* blade) override {
    return !(r_ == 0 && g_ == 0 && b_ == 0);
  }
...
public:
  ...
  RGBA_um getColor(RGBA_um a, RGBA_um b, int led) override {
    ...
    RGBA_um to_color = RGBA_um::Transparent();
    if (cur_ >= ntrans_ - 1) {
      to_color = b;
    } else {
